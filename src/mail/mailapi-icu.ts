...
const MAILAPI_ICU_POLL_ATTEMPTS = 10;
...
async function mailApiIcuFetch(
  input: string | URL,
...
function parseTokenLine(line: string, index: number): MailApiIcuAccount | null {
  ...
  if (!email.includes("@")) {
    throw new Error(
      `MailAPI.ICU 第 ${index + 1} 行邮箱格式不正确: ${emailRaw}`,
    );
  }
  ...
  if (!orderNo) {
    throw new Error(
      `MailAPI.ICU 第 ${index + 1} 行缺少 orderNo 参数: ${apiUrl}`,
    );
  }
...
async function loadAccounts(): Promise<MailApiIcuAccount[]> {
  ...
  try {
    ...
    if (normalizedError.code === "ENOENT") {
      throw new Error(
        `未找到 MailAPI.ICU 账号文件: ${MAILAPI_ICU_TOKENS_FILE}`,
      );
    }
    throw error;
  }
  ...
  if (!accounts.length) {
    throw new Error(
      `未在文件找到可用的 MailAPI.ICU 账号: ${MAILAPI_ICU_TOKENS_FILE}`,
    );
  }
...
async function fetchMessages(
  ...
  try {
    ...
    throw new Error(
      `MailAPI.ICU 返回 JSON 解析失败: ${(error as Error).message}; body=${rawBody}`,
    );
  }
...
async function resolveAccountForEmail(
  email: string,
): Promise<MailApiIcuAccount> {
...
export function createMailApiIcuProvider() {
  return {
    ...
    async getEmailVerificationCode(email: string, options: EmailVerificationCodeRequestOptions = {}) {
      ...
      for (
        let attempt = 1;
        attempt <= MAILAPI_ICU_POLL_ATTEMPTS;
        attempt += 1
      ) {
        console.log(
        ...
        if (attempt < MAILAPI_ICU_POLL_ATTEMPTS) {
          await new Promise((resolve) =>
            setTimeout(resolve, MAILAPI_ICU_POLL_INTERVAL_MS),
          );
        }
      }
